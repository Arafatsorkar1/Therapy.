            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="border shadow p-5">
                        ...
                        <form class="border border-dark p-3" action="{{ route('service-order') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group row">
                                <label class="col-sm-2 col-form-label">Traning Title</label>
                                <div class="col-sm-10">
                                    <input type="text" name="" readonly value="{{ $service->title }}" class="form-control @error('title') is-invalid @enderror">
                                    <input type="hidden" name="service_id" readonly value="{{ $service->id }}" class="form-control @error('service_id') is-invalid @enderror">
                                    <input type="hidden" name="user_id" readonly value="{{ $user->id }}" class="form-control @error('user_id') is-invalid @enderror">
                                    @error('service_id')
                                    <small>{{ $message }}</small>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" value="{{$service->price}}" readonly class="form-control @error('price') is-invalid @enderror">
                                    @error('price')
                                    <small>{{ $message }}</small>
                                    @enderror
                                </div>
